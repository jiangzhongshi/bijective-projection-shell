...
using Quadric = std::tuple<Eigen::MatrixXd, Eigen::RowVectorXd, double>;
    std::function<bool(
      ...
      const int                                                        /*e*/
      )> qslim_in_shell_handle(const PrismCage& pc, const prism::geogram::AABB &tree_B, const prism::geogram::AABB &tree_T, std::vector<std::set<int>> &map_track,int &v1, int &v2, std::vector<Quadric> &quadrics);

TEST_CASE("qslim section")
{
  // Prism Initialize.
  PrismCage pc("../buildr/rocker_arm.obj.h5");
  RowMatd mT, mB;
  RowMati mF;
  ...
  pre_collapse = qslim_in_shell_handle(pc, tree_B, tree_T, track_to_prism,v1,v2,quadrics);
  ...
  igl::write_triangle_mesh("simple_rocker.ply", U, G);
}
