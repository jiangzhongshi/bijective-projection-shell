...
bool prism::project_to_proxy_mesh(const std::array<Vec3d, 9> &stack,
  ...
  prism::phong::fiber_endpoints(stackV_bm, type, tuple[0], tuple[1],
                                endpoints0);
  for (int i = 0; i < 3; i++) {
    if (endpoints0[i] == endpoints0[i + 1])
      continue;
    if (pxtree.segment_hit(endpoints0[i], endpoints0[i + 1], hit))
      return true;
  }
  ...
  prism::phong::fiber_endpoints(stackV_mt, type, tuple[0], tuple[1],
                                endpoints1);
  for (int i = 0; i < 3; i++) {
    if (endpoints1[i] == endpoints1[i + 1])
      continue;
    if (pxtree.segment_hit(endpoints1[i], endpoints1[i + 1], hit))
      return true;
  }
  return false;
}
...
void prism::correspond_bc(const PrismCage &pc, const RowMatd &pxV,
  ...
  for (int i = pc.ref.aabb->num_freeze; i < queryP.rows(); i++) {
    ...
    if (!project_to_proxy_mesh({pc.base[v0], pc.base[v1], pc.base[v2],
                                pc.mid[v0], pc.mid[v1], pc.mid[v2], pc.top[v0],
                                pc.top[v1], pc.top[v2]},
...
bool prism::project_to_ref_mesh(
  ...
  spdlog::trace("combined tracks {}", iterable2str(combined_tracks));
